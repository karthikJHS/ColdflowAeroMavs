...
TWR_list = linspace(1, 10, 10);
n_total = numel(fuel_mass_list) * numel(TWR_list);
results = [];
i_sim = 0;
...
for m_fuel = fuel_mass_list
    for TWR = TWR_list
        ...
        waitbar(i_sim / n_total, wb, ...
            sprintf('Sim %d/%d: Fuel=%.2f, TWR=%.2f', i_sim, n_total, m_fuel, TWR));
        ...
        results = [results; m_fuel, TWR, m_0, thrust, mdot, burn_time, altitude];
        fprintf('[%3d/%3d] Fuel = %.2f kg, TWR = %.2f → Apogee = %.1f m\n', ...
            i_sim, n_total, m_fuel, TWR, altitude);
    end
end
...
result_table = array2table(results, ...
    'VariableNames', {'FuelMass_kg', 'TWR', 'InitialMass_kg', 'Thrust_N', ...
